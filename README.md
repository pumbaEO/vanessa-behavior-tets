...
* Необходимо иметь установленный oscript и доступность его вызова в переменной PATH 
* Установленную 8.3.8 
* все скрипты контрибьютора указанные ниже - сгруппированы в папке `tools`
* В коммандной строке запустить `init.bat/sh` в зависимости от предпочитаемой системы windows/linux
    > Результатом должно быть создан в папке build каталог ib и out, в каталоге ib будет созданна пустая файловая база и загруженна из базовая конфигурация. 

* В коммандной строке необходимо запустить `compile.bat/./compile.sh` для возможности сборки исходников во внешние обработки. 
* Для запуска тестов необходимо запускать `tests.bat/sh` 
* При доработке в конфигураторе внешней обработки необходимо запустить decompile.bat/sh для разборки всех epf файлов.
