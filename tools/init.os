...
Функция ПрочитатьФайлИнформации(ПутьКФайлу)
...
Процедура СоздатьФайловуюБазу(Конфигуратор, Знач КаталогБазы, Знач ПутьКШаблону="", Знач ИмяБазыВСписке="") Экспорт
    ...
    Приложение = Конфигуратор.ПутьКПлатформе1С() + " "+СтрокаЗапуска;
    Попытка
...
Процедура ИнициализацироватьБазуДанных(Знач ПутьКSRC="", Знач ПутьКDT="", Знач СтрокаПодключения="", Знач Пользователь="", Знач Пароль="",
                                        Знач КлючРазрешенияЗапуска = "", Знач ВерсияПлатформы="", Знач ПутьКCFИсходники = "") 
    ...
    КонецЕсли;
    Конфигуратор.УстановитьИмяФайлаСообщенийПлатформы("/tmp/test.log");
    КореньКаталогаGit = СокрЛП(ЗапуститьПроцесс("git rev-parse --show-toplevel"));
    СоздатьКаталог(ОбъединитьПути(КореньКаталогаGit, "build", "out"));    
    Если ПустаяСтрока(СтрокаПодключения) Тогда
        КаталогБазы = ОбъединитьПути(КореньКаталогаGit, "./build/ib");
        ФайлБазы = Новый Файл(КаталогБазы);
        Если ФайлБазы.Существует() Тогда 
            Лог.Отладка("Удаляем файл");
            УдалитьФайлы(ФайлБазы.ПолноеИмя, ПолучитьМаскуВсеФайлы());
        КонецЕсли;
        СоздатьКаталог(ФайлБазы.ПолноеИмя);
        СоздатьФайловуюБазу(Конфигуратор, КаталогБазы);
        //Конфигуратор.СоздатьФайловуюБазу(КаталогБазы);
        СтрокаПодключения = "/F""" + ФайлБазы.ПолноеИмя + """";
    КонецЕсли;
    ...
    Если Не ПустаяСтрока(ПутьКSRC) Тогда
        ...
        runner = Новый runner();
        runner.СобратьИзИсходниковТекущуюКонфигурацию(ПутьКSRC, СтрокаПодключения, Пользователь, Пароль, ВерсияПлатформы);
    КонецЕсли;
