...
Процедура Декомпилировать(Знач Путь, Знач КаталогВыгрузки, Знач КорневойПутьВыходной = "", Знач КорневойПутьВходной = "", Знач СтрокаПодключения = "", Знач Пользователь = "", Знач Пароль = "", Знач ВерсияПлатформы = "") Экспорт
    ...
    КэшИзмененныхФайлов = "";
    Если Файл.ЭтоКаталог() Тогда
        ФайлКэша = Новый Файл(ОбъединитьПути(Файл.ПолноеИмя, "cache.txt"));
        Если ФайлКэша.Существует() Тогда 
            чтение = Новый ЧтениеТекста(ФайлКэша.ПолноеИмя);
            КэшИзмененныхФайлов = чтение.Прочитать();
        КонецЕсли;
        РазобратьКаталог(Файл, КаталогВыгрузки, КорневойПутьВыходной, СтрокаПодключения, Пользователь, Пароль, ВерсияПлатформы);
        
        Запись = Новый ЗаписьТекста(ОбъединитьПути(Файл.ПолноеИмя, "cache.txt"));
        Запись.Записать(КэшИзмененныхФайлов);
        Запись.Закрыть();
        КэшИзмененныхФайлов = "";
    Иначе
...
Функция ФайлНеИзмененн(Файл, Кэш = Неопределено)
    Если Кэш = Неопределено Тогда 
        Кэш = КэшИзмененныхФайлов;
    КонецЕсли;

    СтрокаПоиска = Строка(Файл.ПолноеИмя)+";"+Строка(Файл.ПолучитьВремяИзменения());
    Если СтрНайти(Кэш, СтрокаПоиска) > 0 Тогда
        Возврат Истина;
    КонецЕсли;

    Возврат Ложь;
КонецФункции // ФайлНеИзмененн(Файл, Кэш = Неопределено)

Процедура ДобавитьВКэшИзменения(Файл, Кэш = Неопределено)
    Если Кэш = Неопределено Тогда 
        Кэш = КэшИзмененныхФайлов;
    КонецЕсли;

    СтрокаПоиска = Строка(Файл.ПолноеИмя)+";"+Строка(Файл.ПолучитьВремяИзменения());
    Кэш = Кэш + Символы.ПС + СтрокаПоиска;
...
Процедура РазобратьКаталог(Знач ОбъектКаталога, Знач КаталогВыгрузки, Знач КаталогКорень, Знач СтрокаПодключения = "", Знач Пользователь = "", Знач Пароль = "", Знач ВерсияПлатформы="") Экспорт
    ...
    Для Каждого Файл из Файлы Цикл
        ...
        ИначеЕсли ТипФайлаПоддерживается(Файл) Тогда
            ...
            Если ФайлНеИзмененн(Файл, КэшИзмененныхФайлов) Тогда
                Продолжить;
            Иначе 
                ДобавитьВКэшИзменения(Файл, КэшИзмененныхФайлов);
            КонецЕсли;
...
Функция Компилировать(Знач Путь, Знач КаталогВыгрузки, Знач СтрокаПодключения = "", Знач Пользователь = "", Знач Пароль = "", Знач ВерсияПлатформы = "") Экспорт
    ...
    Если Рекурсивно Тогда
        ...
        ЗаписьТекста = Новый ЗаписьТекста(ОбъединитьПути(КаталогВыгрузки, "cache.txt"));
        СписокФайловВКаталоге = НайтиФайлы(КаталогВыгрузки, ПолучитьМаскуВсеФайлы(), Истина);
        Для Каждого Файл Из СписокФайловВКаталоге Цикл
            Если Файл.ЭтоКаталог() = Ложь Тогда
                ЗаписьТекста.ЗаписатьСтроку(Строка(Файл.ПолноеИмя)+";"+Строка(Файл.ПолучитьВремяИзменения()));
            КонецЕсли;
        КонецЦикла;
