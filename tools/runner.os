...
Процедура СобратьИзИсходниковТекущуюКонфигурацию(Знач ВходнойКаталог,  Знач СтрокаПодключения="", Знач Пользователь="", Знач Пароль="", Знач ВерсияПлатформы="", Знач СписокФайловДляЗагрузки = "", Знач СниматьСПоддержки = Истина) Экспорт 
    ...
    Если НЕ ПустаяСтрока(СписокФайловДляЗагрузки) Тогда
        ...
        Если Лог.Уровень() <= УровниЛога.Отладка  Тогда
            Лог.Информация("Загружаем по списку файлов:");
...
Процедура СледитьЗаИзменениямиEPF(Значение, КэшПутей)
    ...
    СтрокаПодключения = Значение.Получить("connectionstring");
    ...
    Если Кэш = Неопределено Тогда
        ...
        КонецЦикла;
        СписокФайлов = НайтиФайлы(КаталогИсходящий, ПолучитьМаскуВсеФайлы(), Истина);
        Для каждого Файл из СписокФайлов Цикл
            Кэш.Вставить(Файл.ПолноеИмя, Файл.ПолучитьВремяИзменения());
        КонецЦикла; 
    ...
    Для каждого Файл из СписокФайлов Цикл
        ...
        //Лог.Отладка("Проверяем:"+Файл.ПолноеИмя + " время старое:"+Строка(Время) + " новое:"+ ВремяТекущее);
        ...
        Если Изменен = Истина Тогда
            ...
            ИначеЕсли ОбработкуНашли = Истина Тогда 
                КаталогВыгрузкиОбработки = Новый Файл(ОбъединитьПути(КаталогИсходящий, ПолучитьОтносительныйПутьФайла(КаталогВходящий, ПапкаИсходников), "../")).ПолноеИмя;
                КаталогФайл = Новый Файл(ОбъединитьПути(КаталогИсходящий, ПолучитьОтносительныйПутьФайла(КаталогВходящий, ПапкаИсходников)));
                ...
                Для каждого ЭлементОбработки из СписокСобранныхОбработок Цикл 
                    ...
                    СписокОбработаных.Вставить(ЭлементОбработки.ПолноеИмя, Истина);    
                КонецЦикла;
                СписокОбработаных.Вставить(ПапкаИсходников, Истина);
            Иначе
                НовыйПутьВыгрузки = ОбъединитьПути(КаталогИсходящий, ПолучитьОтносительныйПутьФайла(КаталогВходящий, Файл.ПолноеИмя));
    ...
    Для каждого Файл из СписокФайлов Цикл
        ...
        Если Время = Неопределено ИЛИ Время <> ВремяТекущее Тогда 
            Изменен = Истина;
            Лог.Информация("Изменен:"+Файл.ПолноеИмя + " время старое:"+Строка(Время) + " новое:"+Файл.ПолучитьВремяИзменения());
        КонецЕсли;

        Если Изменен = Истина Тогда
            Если ТипФайлаПоддерживается(Файл) Тогда
                Лог.Информация("Разбираем внешнюю обработку");
                НовыйПутьВыгрузки = ОбъединитьПути(КаталогВходящий, ПолучитьОтносительныйПутьФайла(КаталогИсходящий, Файл.Путь));
                СоздатьКаталог(НовыйПутьВыгрузки);
                КаталогРазобранный = РазобратьФайлВнешняяОбработка(Файл, НовыйПутьВыгрузки, КаталогИсходящий, СтрокаПодключения, Пользователь, Пароль, ВерсияПлатформы);
            ...
            Иначе
                ОтносительныйПутьКаталогаФайла = ПолучитьОтносительныйПутьФайла(КаталогИсходящий, Файл.ПолноеИмя);
                Лог.Отладка("ОтносительныйПутьКаталогаФайла <"+ОтносительныйПутьКаталогаФайла+">");
                ПутьФайлаИсходников = ОбъединитьПути(КаталогВходящий, ОтносительныйПутьКаталогаФайла);
                Лог.Отладка("ПутьФайлаИсходников <"+ПутьФайлаИсходников+">");
...
Процедура СледитьЗаИзменениямиИсходниковCF(Значение, КэшПутей)
...
Процедура Следить(Знач ФайлНастроек = "")
    Перем КэшПутей;
    ...
    Пока Истина Цикл 
        Для каждого Элемент ИЗ Настройки Цикл
            ...
            Если Значение <> Неопределено Тогда
                СледитьЗаИзменениямиEPF(Значение, КэшПутей)
            КонецЕсли;
...
Функция ЗапускВКоманднойСтроке()
    ...
    Попытка
        ...
        ИначеЕсли Команда = ВозможныеКоманды().Следить Тогда 
            Следить(Аргументы.ЗначенияПараметров["ПутьКФайлуНастроек"]);
        КонецЕсли;
