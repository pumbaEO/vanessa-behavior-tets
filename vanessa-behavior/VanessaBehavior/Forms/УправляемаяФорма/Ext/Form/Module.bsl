...
Функция ПреобразоватьПутьСТочкамиКНормальномуПути(ОригСтр)
	...
	Пока Истина Цикл
		Стр = Текст.ПрочитатьСтроку();
		Если Стр = Неопределено ИЛИ ПустаяСтрока(СокрЛП(Стр)) Тогда
			Прервать;
		КонецЕсли;	 
		Рез = СокрЛП(Стр);
	КонецЦикла;	
...
Процедура ПреобразоватьПараметрыКоторыеНачинаютсяСТочкиКНормальнымПутям(СтруктураПараметров)
	...
	Для каждого Ключ Из МассивКлючей Цикл
		...
		сообщить("ПреобразоватьПараметрыКоторыеНачинаютсяСТочкиКНормальнымПутям:"+Стало) ;
	...
	Если StartFeaturePlayer = Истина Тогда
		...
		Если ТипЗнч(ЗначКаталогиБиблиотек) = Тип("Массив") Тогда
			Для Сч = 0 По ЗначКаталогиБиблиотек.Количество()-1 Цикл
				ЗначКаталогиБиблиотек[Сч] = ПолучитьПолныйПутьОтносительноКаталогаОбработки(ЗначКаталогиБиблиотек[Сч], ПутьКОбработке);
		...
		Если Объект.ДелатьСкриншотПриВозникновенииОшибки Тогда
			...
			Объект.КаталогOutputСкриншоты = ПолучитьПолныйПутьОтносительноКаталогаОбработки(ЗнКаталогOutputСкриншоты, ПутьКОбработке);
		КонецЕсли;	 
		...
		Если Объект.СоздаватьИнструкциюHTML Тогда
			...
			Объект.КаталогOutputИнструкцияHTML = ПолучитьПолныйПутьОтносительноКаталогаОбработки(ЗнКаталогOutputИнструкцияHTML, ПутьКОбработке);
		КонецЕсли;	 
		...
		Если Объект.СоздаватьИнструкциюMarkdown Тогда
			...
			Объект.КаталогOutputИнструкцияMarkdown = ПолучитьПолныйПутьОтносительноКаталогаОбработки(ЗнКаталогOutputИнструкцияMarkdown, ПутьКОбработке);
		КонецЕсли;	 
		...
		Если Объект.СоздаватьИнструкциюВидео Тогда
			...
			Объект.КаталогOutputИнструкцияВидео = ПолучитьПолныйПутьОтносительноКаталогаОбработки(ЗнКаталогOutputИнструкцияВидео, ПутьКОбработке);
		КонецЕсли;	 
		...
		Если ЗначениеЗаполнено(ЗнКаталогСкриптовСборкиВидео) Тогда
			Объект.КаталогСкриптовСборкиВидео = ПолучитьПолныйПутьОтносительноКаталогаОбработки(ЗнКаталогСкриптовСборкиВидео, ПутьКОбработке);
		КонецЕсли;	 
		...
		Если ЗнДелатьОтчетВФорматеxUnit = Истина Тогда
			...
			Если ЗначениеЗаполнено(ЗнКаталогOutputxUnit) Тогда
				Файл = Новый Файл(ПолучитьПолныйПутьОтносительноКаталогаОбработки(ЗнКаталогOutputxUnit, ПутьКОбработке));
				Если Не ФайлСуществуетКомандаСистемы(Файл.ПолноеИмя, "ЗнКаталогOutputxUnit") Тогда
					...
					Возврат;
				КонецЕсли;	 
				Объект.КаталогOutputxUnit      = ПолучитьПолныйПутьОтносительноКаталогаОбработки(ЗнКаталогOutputxUnit, ПутьКОбработке);
			...
			Если ЗнПутьКФайлуДляВыгрузкиСтатусаВыполненияСценариев <> Неопределено Тогда
				Объект.ПутьКФайлуДляВыгрузкиСтатусаВыполненияСценариев = ПолучитьПолныйПутьОтносительноКаталогаОбработки(ЗнПутьКФайлуДляВыгрузкиСтатусаВыполненияСценариев, ПутьКОбработке);
			КонецЕсли;	 
			...
			Если ЗначениеЗаполнено(ЗнКаталогOutputxUnitБазовый) Тогда
				...
				Файл = Новый Файл(ПолучитьПолныйПутьОтносительноКаталогаОбработки(ЗнКаталогOutputxUnitБазовый, ПутьКОбработке));
				Если Не ФайлСуществуетКомандаСистемы(Файл.ПолноеИмя, "ЗнКаталогOutputxUnitБазовый") Тогда
					...
					СоздатьКаталогКомандаСистемы(ПолучитьПолныйПутьОтносительноКаталогаОбработки(ЗнКаталогOutputxUnitБазовый, ПутьКОбработке));
				КонецЕсли;	 
				
				СисИнфо = Новый СистемнаяИнформация; 
				ВерсияПриложения = СисИнфо.ВерсияПриложения;
				
				ЗнКаталогOutputxUnitБазовый = ПолучитьПолныйПутьОтносительноКаталогаОбработки(ДополнитьСлешВПуть(ЗнКаталогOutputxUnitБазовый) + ПолучитьЗначениеПереданногоПараметра(СтрутктураJSON,"ИмяСборки"), ПутьКОбработке);
		...
		Если ЗнДелатьОтчетВФорматеАллюр = Истина Тогда
			...
			Если ЗначениеЗаполнено(ЗнКаталогOutputAllure) Тогда
				УдалитьКаталогКомандаСистемы(ПолучитьПолныйПутьОтносительноКаталогаОбработки(ЗнКаталогOutputAllure, ПутьКОбработке));
				
				Файл = Новый Файл(ПолучитьПолныйПутьОтносительноКаталогаОбработки(ЗнКаталогOutputAllure, ПутьКОбработке));
			...
			Сообщить(ЗнКаталогOutputAllureБазовый);
			Если ЗначениеЗаполнено(ЗнКаталогOutputAllureБазовый) Тогда
				Файл = Новый Файл(ПолучитьПолныйПутьОтносительноКаталогаОбработки(ЗнКаталогOutputAllureБазовый, ПутьКОбработке));
				...
				Если ПолучитьЗначениеПереданногоПараметра(СтрутктураJSON,"СоздаватьПодкаталогВКаталогеAllureДляЭтойСборки") = Истина Тогда
					ЗнКаталогOutputAllureБазовый = ПолучитьПолныйПутьОтносительноКаталогаОбработки(ЗнКаталогOutputAllureБазовый + "/" + ПолучитьЗначениеПереданногоПараметра(СтрутктураJSON,"ИмяСборки"), ПутьКОбработке);					
			...
			Если Объект.ДелатьСкриншотПриВозникновенииОшибки Тогда
				Для каждого РезультатПрохожденияТестовСценария Из МассивРезультатПрохожденияТестовСценария Цикл
					...
					Для каждого СтруктураСкриншот Из РезультатПрохожденияТестовСценария.МассивСкриншотов Цикл
						...
						КопироватьФайлКомандаСистемы(ВременныйФайл, ПолучитьПолныйПутьОтносительноКаталогаОбработки(Объект.КаталогOutputAllure, ПутьКОбработке) + "/" + СтруктураСкриншот.ИмяФайла);
					КонецЦикла;
				КонецЦикла;
			КонецЕсли;	 
...
Функция ФайлСуществуетКомандаСистемы(Знач ИмяФайла,ДопПараметры = Неопределено) Экспорт
	...
	Если ЕстьПоддержкаАсинхронныхВызовов Тогда
		...
		Иначе 
            ...
            Исключение
                Сообщить("ФайлСуществуетКомандаСистемы:"+ОписаниеОшибки());
            КонецПопытки;
...
Процедура СоздатьФайлыОбработок(БылиОшибки,СтруктураОписанияEpf,ДополнительныеПараметры = Неопределено)
	...
	Если ГенерироватьУФ Тогда
		...
		Если СтруктураОписанияEpf.ФайлEpfПересоздавался Тогда
			...
			Если Не ФайлСуществуетКомандаСистемы(ФайлИмяФайлаФормаУФ.ПолноеИмя, "СоздатьФайлыОбработок") Тогда
	...
	Если ФайлСуществуетКомандаСистемы(ИмяФайлОсноваОбработкиTemplate, "СоздатьФайлыОбработок") Тогда
	...
	Если ФайлСуществуетКомандаСистемы(ФайлВременногоEPF.ПолноеИмя,"СоздатьФайлыОбработок" ) Тогда
		Если ФайлСуществуетКомандаСистемы(ФайлРеальногоEPF.ПолноеИмя, "СоздатьФайлыОбработок") Тогда
...
Процедура ПодсветитьАктивноеОкно(Длительность,ЖдатьЗавершения = Истина) Экспорт
	...
	Если Не ФайлСуществуетКомандаСистемы(ПутьКПриложению, "ПодсветитьАктивноеОкно") Тогда
		Возврат;
	КонецЕсли;	 
